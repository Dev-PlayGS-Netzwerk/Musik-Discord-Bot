    ...
    execute(client, message, args) {
        ...
        if (args.join(" ").toLowerCase() === 'queue') {
            if (client.player.getQueue(message).loopMode) {
                ...
                return message.channel.send(`${client.emotes.success} - Wiederholungs Modus **deaktiviert** !`);
            } else {
                client.player.setLoopMode(message, true);
                return message.channel.send(`${client.emotes.success} - Wiederholungs Modus **aktiviert** die jetzige Warteschlange wird unendlich lange wiederholt!`);
            };
        } else {
